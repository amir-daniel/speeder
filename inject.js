...
var tc = {
  settings: {
    ...
    displayKeyCode: 68, // default: D
    rememberSpeed: false, // default: false
    forceLastSavedSpeed: false, //default: false
    audioBoolean: false, // default: false
    startHidden: false, // default: false
    controllerOpacity: 1, // default: 1
    keyBindings: [],
    blacklist: `\
    www.instagram.com
    twitter.com
...
chrome.storage.sync.get(tc.settings, function (storage) {
  ...
  if (storage.keyBindings.length == 0) {
    ...
    tc.settings.keyBindings.push({
      ...
      value: Number(storage.speedStep) || 0.05,
      force: false,
      predefined: true
    }); // default S
    tc.settings.keyBindings.push({
  ...
  if (
    ...
    tc.settings.keyBindings.push({
      action: "display",
      key: Number(storage.displayKeyCode) || 68,
      value: 0,
      force: false,
      predefined: true
    }); // default V
...
function convertNumToPct(speed) {
...
function defineVideoController() {
  ...
  tc.videoController.prototype.initializeControls = function () {
    ...
    const speedPct = convertNumToPct(this.video.playbackRate);
    ...
    var shadowTemplate = `
      ...
      tc.settings.controllerOpacity
          ...
          <span data-action="drag" class="draggable">${speedPct}</span>
          <span id="controls">
            <button data-action="rewind" class="rw">&larr;</button>
            <button data-action="slower">&minus;</button>
            <button data-action="faster">&plus;</button>
            <button data-action="advance" class="rw">&rarr;</button>
...
function setupListener() {
  ...
  function updateSpeedFromEvent(video) {
    ...
    var speedPct = convertNumToPct(speed);

    log("Playback rate changed to " + speed, 4);

    log("Updating controller with new speed", 5);
    speedIndicator.textContent = speedPct;
...
function setSpeed(video, speed) {
  ...
  let speedFraction = speed.toFixed(2);
  let speedPct = convertNumToPct(speed);
  if (tc.settings.forceLastSavedSpeed) {
    video.dispatchEvent(
      new CustomEvent("ratechange", {
        detail: { origin: "videoSpeed", speed: speedFraction }
      })
    );
  } else {
    video.playbackRate = Number(speedFraction);
  }
  var speedIndicator = video.vsc.speedIndicator;
  speedIndicator.textContent = speedPct;
  tc.settings.lastSpeed = speed;
